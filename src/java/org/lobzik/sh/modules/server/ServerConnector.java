...
public class ServerConnector extends Thread 
{
	...
	private static ServerReader reader = null;
	private static ServerPoller poller = null;
	private static Logger log = null;
	private static ServerCommandHandler commandHandler = null;
	private static final ServerConnector instance = new ServerConnector();
	
	private ServerConnector() {}
	
	public static ServerConnector getInstance(String deviceIp, int port, int afterLostPeriod, int attemptsPeriod, int tucPeriod, int timeout)
	{
	...
	{
		if (reader != null)
			return reader.getServerReplies();
	...
	public synchronized void run() 
	{
		while(run)
		{
			try 
			{
				...
				reader = new ServerReader(inFromServer, this, commandHandler, log);
				reader.start();
				poller = new ServerPoller(outToServer, this);
				poller.start();
				connectErrorMessage = "";
				connectTries = 0;
				connected = true;
